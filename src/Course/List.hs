...
seqOptional xs = let unwrapped = unwrap xs in
              if length unwrapped == length xs
              then Full unwrapped
              else Empty

unwrap :: List (Optional a) -> List a
unwrap (Full x :. xs) = x :. unwrap xs
unwrap Nil = Nil
unwrap (Empty :. xs) = unwrap xs
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = count 0 xs
count :: Int -> List a -> Bool
