...
-- >>> instance Arbitrary a => Arbitrary (ListZipper a) where arbitrary = do (l :: List a) <- arbitrary; x <- arbitrary; r <- arbitrary; P.return (ListZipper l x r)
...
instance Functor ListZipper where
  (<$>) f (ListZipper ls curr rs) = ListZipper (f <$> ls) (f curr) (f <$> rs)
...
instance Functor MaybeListZipper where
  (<$>) f (IsZ (ListZipper ls curr rs)) = IsZ $ ListZipper (f <$> ls) (f curr) (f <$> rs)
  (<$>) _ _ = IsNotZ
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper ls curr rs) = reverse ls ++ (return curr) ++ rs
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper ls foc rs) = ListZipper ls (f foc) rs
...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus new (ListZipper ls _ rs) = ListZipper ls new rs
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper ls foc rs) = case break p ls of
                                      (left, foc' :. right) -> IsZ $ ListZipper right foc' $ (reverse left) ++ (foc :. rs)
                                      (_, Nil)              -> IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper ls foc rs) = case break p rs of
                                       (left, foc' :. right) -> IsZ $ ListZipper (reverse left ++ (foc :. ls)) foc' right
                                       (_, Nil)              -> IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (l :. ls) foc rs) = ListZipper ls l (foc :. rs)
moveLeftLoop (ListZipper Nil foc rs)       =
  let (foc' :. ls) = reverse $ foc :. rs
  in ListZipper ls foc' Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls foc (r :. rs)) = ListZipper (foc :. ls) r rs
moveRightLoop (ListZipper ls foc Nil)       =
  let (foc' :. rs) = reverse $ foc :. ls
  in ListZipper Nil foc' rs
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (l :. ls) foc rs) = IsZ $ ListZipper ls l (foc :. rs)
moveLeft _                             = IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper ls foc (r :. rs)) = IsZ $ ListZipper (foc :. ls) r rs
moveRight _                             = IsNotZ
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (l :. ls) foc rs) = IsZ $ ListZipper (foc :. ls) l rs
swapLeft _                             = IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper ls foc (r :. rs)) = IsZ $ ListZipper ls r (foc :. rs)
swapRight _                             = IsNotZ
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ foc rs) = ListZipper Nil foc rs
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper ls foc _) = ListZipper ls foc Nil
...
moveLeftN n lz | n < 0     = moveRightN (-n) lz
               | otherwise = case lz of
                  ListZipper (l :. ls) foc rs -> moveLeftN (n - 1) $ ListZipper ls l (foc :. rs)
                  ListZipper Nil _ _          -> IsNotZ
...
moveRightN n lz | n < 0     = moveLeftN (-n) lz
                | otherwise = case lz of
                    ListZipper ls foc (r :. rs) -> moveRightN (n - 1) $ ListZipper (foc :. ls) r rs
                    ListZipper _ _ Nil          -> IsNotZ
...
moveLeftN' 0 lz = Right lz
moveLeftN' n lz | n > 0 = case moveLeftN n lz of
                            IsZ lz' -> Right lz'
                            IsNotZ  -> Left $ length $ lefts lz
                | n < 0 = case moveRightN (-n) lz of
                            IsZ lz' -> Right lz'
                            IsNotZ  -> Left $ length $ rights lz
                | otherwise = error "v bad"
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz = let pos = length $ lefts lz
           in moveRightN (n - pos) lz
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lz = case nth (index lz + (length $ rights lz)) lz of
           IsZ lz' -> lz'
           IsNotZ  -> error "should not happen"
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper ls foc rs) = ListZipper (foc :. ls) x rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper ls foc rs) = ListZipper ls x (foc :. rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lfs focf rfs) (ListZipper ls foc rs) = let left   = zipWith ($) lfs ls
                                                               right  = zipWith ($) rfs rs
                                                               newFoc = focf foc
                                                           in ListZipper left newFoc right
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper (unfoldr (unZip moveLeft) lz) (f lz) (unfoldr (unZip moveRight) lz)
    where
      unZip fun lzip = case fun lzip of
                      IsZ lzip' -> Full (f lzip', lzip')
                      IsNotZ  -> Empty
...
instance Traversable ListZipper where
  traverse fun (ListZipper ls foc rs) = let fls  = traverse fun ls
                                            ffoc = fun foc
                                            frs  = traverse fun rs
                                        in (pure ListZipper) <*> fls <*> ffoc <*> frs
...
instance Traversable MaybeListZipper where
  traverse fun (IsZ lz) = (\x -> IsZ x) <$> traverse fun lz
